...
def limit_annual_nuclear_operation(n, snapshots, snakemake):
    ...
    nuclear_generators = n.links[n.links.carrier == "nuclear"].index
    total_generation = (
        n.model["Link-p"].sel(name=nuclear_generators)
        * n.snapshot_weightings["generators"].to_xarray()
    ).sum("snapshot")
    max_generation = (
        n.links.loc[nuclear_generators, "p_nom"].rename_axis(index="name").to_xarray()
        * n.snapshot_weightings["generators"].to_xarray()
    ).sum("snapshot")
