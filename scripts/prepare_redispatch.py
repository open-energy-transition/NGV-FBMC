...
def _apply_multiplier(
    ...
    if undefined_multipliers:
        logger.warning(
            f"Neither {direction} multiplier nor strike price provided for the carriers: {sorted(undefined_multipliers)}"
        )
    ...
    for comp in base_network.components[["Generator", "StorageUnit", "Link"]]:
        ...
        if not g_multilink.empty:
            ...
            up_limit = (
                ...
                .ffill()
                - p_gb
            )[fuel_updown_gens.index] / p_nom
            down_limit = -p_gb[fuel_updown_gens.index] / p_nom
            ...
            emission_gb = (-1) * dispatch_result.c.links.dynamic.p2[idx].sum(
                axis="columns"
            )
            # this is not working; need to set e_set instead below
            base_network.c.stores.dynamic.p_set.loc[:, "co2 atmosphere"] = emission_gb
            base_network.c.stores.dynamic.e_set.loc[:, "co2 atmosphere"] = (
