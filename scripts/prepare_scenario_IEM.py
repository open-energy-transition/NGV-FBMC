...
def add_waste_element(
	...
	cc_adjustment = (eur.generators[eur.generators.carrier == 'solid biomass'].capital_cost.iloc[0]) / (gb.generators[gb.generators.carrier =='biomass'].capital_cost.iloc[0])
	mc_adjustment = (eur.generators[eur.generators.carrier == 'solid biomass'].marginal_cost.iloc[0]) / (gb.generators[gb.generators.carrier =='biomass'].marginal_cost.iloc[0])
...
def add_co2_multilink(
	...
	for gb_carrier, eur_carrier in carrier_map["Generator"].items():
		...
		ref = eur.links[(eur.links.carrier == eur_carrier) & (eur.links.bus1.str.startswith("GB"))]
...
def align_tech_econ_assumptions(
	...
	for comp in ['Store', 'StorageUnit', 'Links']:
		...
		cols = [col for col in eur.storage_units.columns if re.search('cost|efficiency|loss', col)]
		...
		for carrier_gb, carrier_eur in carriers.items():
			...
			eur_carrier_mask = eur.c[comp].static[eur.c[comp].static.carrier == carrier_eur]
